...
\`\`\``);
  ...
  it('keeps renderer source free of direct eval/new Function execution', () => {
    const rendererPath = fs.existsSync(path.join(process.cwd(), 'src/modules/renderer.ts'))
      ? 'src/modules/renderer.ts'
      : 'src/modules/renderer.js';
    const chatPath = fs.existsSync(path.join(process.cwd(), 'src/modules/chat.ts'))
      ? 'src/modules/chat.ts'
      : 'src/modules/chat.js';
    const source = fs.readFileSync(path.join(process.cwd(), rendererPath), 'utf8');
    const chatSource = fs.readFileSync(path.join(process.cwd(), chatPath), 'utf8');
