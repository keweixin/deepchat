...
import { ACTOR_STATUS, RUN_STATUS, getActorRoleForTool, TraceRecorder } from './agent-trace.js';

// ─── Actor Display Metadata ─────────────────────────────────────────────────

export interface ActorDisplayMeta {
  id: string;
  label: string;
  title: string;
  icon: string;
...
export const ACTOR_DISPLAY_META: Record<string, ActorDisplayMeta> = Object.freeze({
  planner: {
    id: 'planner',
    label: 'Planner',
    title: '计划员',
...
export const ACTOR_ROLE_IDS: readonly string[] = Object.freeze(Object.keys(ACTOR_DISPLAY_META));
...
export const STATUS_DISPLAY: Record<string, StatusDisplay> = Object.freeze({
...
export interface ActorState extends ActorDisplayMeta {
  status: string;
 ...
 */
export function buildActorState(recorder: TraceRecorder | null): ActorState[] {
  if (!recorder) return [];
...
export interface CrewHeaderState {
  status: string;
  label: string;
  detail: string;
  waitingCount: number;
 ...
 */
export function buildCrewHeaderState(recorder: TraceRecorder | null): CrewHeaderState {
  if (!recorder) {
 ...
 */
export function buildToolCallTimeline(recorder: TraceRecorder | null): ToolCallTimelineItem[] {
  if (!recorder) return [];
 ...
 */
export function buildEventTimeline(recorder: TraceRecorder | null): TimelineEvent[] {
  if (!recorder) return [];
 ...
 */
export function buildContextSnapshots(recorder: TraceRecorder | null): ContextSnapshot[] {
  if (!recorder) return [];
  const summary = recorder.toRunSummary();
  return ((summary.contextSnapshots || []) as Record<string, unknown>[]).map((snap) => ({
    ...snap,
    atFormatted: new Date(snap.at as number).toLocaleTimeString(),
    description: describeCompactionTrigger(snap.trigger as string),
  }));
}

function describeCompactionTrigger(trigger: string): string {
...
export interface LegacyCrewActor {
  id: string;
  label: string;
  icon: string;
  title: string;
 ...
 */
export function toLegacyCrewFormat(actors: ActorState[]): LegacyCrewActor[] {
...
function legacyStatusFromActor(status: string): string {
  const map: Record<string, string> = {
 ...
 */
export function shouldShowCrew(recorder: TraceRecorder | null, displayMode = 'auto'): boolean {
 ...
 */
export function buildHumanReadableSummary(recorder: TraceRecorder | null): string {
  if (!recorder) return '';
  const actors = buildActorState(recorder);
  ...
  const lines: string[] = [
    ...
    `Events: ${(summary as Record<string, unknown>).eventCount}`,
