...
function decorateMermaidDiagrams(container) {
...
function exportTableCsv(wrapper) {
  const table = wrapper?.querySelector('table');
  if (!table) return;
  const rows = [...table.querySelectorAll('tr')].map((row) =>
    [...row.children].map((cell) => csvEscape(cell.textContent.trim())).join(',')
  );
  downloadText(rows.join('\n'), 'deepchat-table.csv', 'text/csv;charset=utf-8');
}

function downloadMermaid(wrapper, format) {
...
function decorateReadableContent(container) {
...
function resetAdaptiveDecorations(container) {
...
function decorateLists(container) {
...
function classifyAnswer(container) {
...
function classifyTableToken(token) {
  ...
  const rows = (token?.rows || []).map((row) => row.map(getTableCellText));
...
function firstMeaningfulChild(container) {
  return [...container.children].find((child) => normalizeWhitespace(child.textContent));
}
