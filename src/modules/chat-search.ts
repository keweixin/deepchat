...
export function buildChatSearchIndex(root: Document | HTMLElement = document) {
  ...
  return messages.map((messageEl, position) => {
    ...
    return {
      messageId: el.dataset.messageId || el.dataset.messageIndex || String(position),
      messageIndex: el.dataset.messageIndex || String(position),
...
export function findChatSearchMatches(index: any[], query: string, options: Record<string, any> = {}) {
...
export function clearChatSearchHighlights(root = document) {
  ...
  root.querySelectorAll('.chat-search-match').forEach((messageEl) => {
    (messageEl as HTMLElement).classList.remove('chat-search-match');
  });
}

export function highlightChatSearchMatches(matches: any[], query: string, options: Record<string, any> = {}) {
...
function getMessageRole(messageEl: HTMLElement) {
