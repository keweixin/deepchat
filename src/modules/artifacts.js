 ...
 * @param {string[]} [options.types] — artifact types to extract (default: all)
 * @param {number} [options.maxArtifacts] — max total artifacts
 * @param {number} [options.maxSourceChars] — max source length per artifact
 */
export function extractArtifacts(markdown, options = {}) {
...
function createMermaidArtifact(source, index) {
  const truncated = source.length >= DEFAULT_MAX_SOURCE_CHARS;
  return {
...
function createTableArtifact(source, index, language) {
  const truncated = source.length >= DEFAULT_MAX_SOURCE_CHARS;
  return {
...
function createJsonArtifact(source, index) {
  let parsed = null;
  try {
...
function createCodeFileArtifact(source, index, language) {
  const extMap = {
    javascript: 'js',
    typescript: 'ts',
    python: 'py',
...
function extractMarkdownTables(source, options) {
