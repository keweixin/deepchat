...
export function exportConversation(conversation: any, format = 'markdown'): void {
...
export function buildConversationMarkdown(conversation: any, options: { onlyFavorites?: boolean } = {}): string {
  const messages = options.onlyFavorites
    ? conversation.messages.filter((message: any) => message.role === 'assistant' && message.favorite)
...
function buildMessageAuditMarkdown(message: any = {}): string {
  let markdown = '';
...
function buildMessageUsageMarkdown(message: any = {}): string {
...
export function buildConversationHtml(conversation: any): string {
...
export function buildToolEvidence(conversation: any): Record<string, unknown> {
  const toolRuns: any[] = [];
...
function getMessageToolEvidenceRuns(message: any = {}): any[] {
...
function buildCacheEvidence(messages: any[] = []): any[] {
  return messages
...
function buildEvidenceCitationStatus(evidence: any = {}, content = ''): any {
  const refs = collectEvidenceRefs(evidence);
  const checked = refs.map((ref: any) => ({
    ...ref,
    cited: isEvidenceRefMentioned(content, ref),
  }));
  const cited = checked.filter((ref: any) => ref.cited).length;
...
function formatCitationStatusText(status: any = {}): string {
...
function collectEvidenceRefs(evidence: any = {}): any[] {
  const refs: any[] = [];
...
function isEvidenceRefMentioned(content = '', ref: any = {}): boolean {
...
function dedupeEvidenceRefs(refs: any[] = []): any[] {
...
function aggregateMessageUsage(messages: any[] = []): any {
...
function normalizeExportUsage(usage: any = null): any {
...
export function buildAssetManifestHtml(conversation: any): string {
...
function collectAssets(conversation: any): any[] {
  const assets: any[] = [];
...
function printConversationAsPdf(conversation: any): void {
