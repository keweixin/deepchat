...
import { exportTraceAsBlob, generateTraceFileName } from './agent-trace.js';
...
export function openTraceInspector(recorder, options = {}) {
  if (!recorder) return;
...
export function toggleTraceInspector(recorder) {
  if (_isOpen) {
    closeTraceInspector();
  } else if (recorder) {
...
function _render(recorder, options = {}) {
  const { conversationTitle = '' } = options;
  const summary = buildCrewHeaderState(recorder);
  const actors = buildActorState(recorder);
  const tools = buildToolCallTimeline(recorder);
  const events = buildEventTimeline(recorder);
  const snapshots = buildContextSnapshots(recorder);
...
function _renderRunSummary(summary) {
  const bar = document.createElement('div');
  bar.className = 'trace-run-summary';

  const items = [
    { label: 'Status', value: _renderStatusDot(summary.status) + summary.label },
...
function _renderSection(title, content, { badge, defaultOpen = true } = {}) {
...
function _renderActorGrid(actors) {
...
function _renderToolCards(tools) {
...
function _renderEventTimeline(events) {
...
function _renderSnapshots(snapshots) {
...
function _renderExportActions(recorder) {
