...
export function renderWidgets(container: HTMLElement): void {
  container.querySelectorAll('.widget-placeholder:not([data-rendered])').forEach((placeholder) => {
    (placeholder as HTMLElement).dataset.rendered = '1';
    const config = parseWidgetConfig((placeholder as HTMLElement).dataset.widget || '');

    if (!config.ok) {
      placeholder.replaceWith(createWidgetError(config.error || '配置解析失败'));
      return;
    }
...
function parseWidgetConfig(encoded: string): ParseResult {
  try {
...
function createWidget(config: WidgetConfig): HTMLElement {
...
function createShell(title: string, subtitle: string): HTMLElement {
...
function createMortgageCalculator(config: WidgetConfig): HTMLElement {
...
function createChartWidget(config: WidgetConfig, mode: 'bar' | 'line'): HTMLElement {
  const items = normalizeChartData(config.data);
  const shell = createShell(
    (config.title as string) || (mode === 'bar' ? '数据柱状图' : '数据折线图'),
  ...
  svg.setAttribute('aria-label', (config.title as string) || '数据图表');
  chart.append(svg);
  shell.append(chart);
...
function createProjectileDemo(config: WidgetConfig): HTMLElement {
...
function normalizeChartData(data: unknown): ChartItem[] {
  ...
  return data
    .map((item, index) => {
      ...
      return {
        label: String(
          (item as Record<string, unknown>).label ?? (item as Record<string, unknown>).name ?? index + 1
        ).slice(0, 24),
        value: Number(
    ...
    .filter((item): item is ChartItem => item !== null && Number.isFinite(item.value))
    .slice(0, 16);
}

function drawBarChart(svg: SVGElement, items: ChartItem[]) {
  svg.replaceChildren();
  const width = 640;
  const height = 300;
  const pad = 42;
...
function drawLineChart(svg: SVGElement, items: ChartItem[]) {
  svg.replaceChildren();
  const width = 640;
  const height = 300;
  const pad = 42;
...
function drawProjectile(svg: SVGElement, velocity: number, angle: number, flight: number, range: number) {
  svg.replaceChildren();
  const width = 640;
  const height = 300;
  const pad = 36;
...
function drawAxes(svg: SVGElement, width: number, height: number, pad: number) {
...
function drawZeroLine(svg: SVGElement, width: number, pad: number, y: number) {
...
function drawLabel(svg: SVGElement, text: string, x: number, y: number) {
...
function createWidgetError(message: string): HTMLElement {
