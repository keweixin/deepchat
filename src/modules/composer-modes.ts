...
export const COMPOSER_MODES: Readonly<Record<string, ComposerMode>> = Object.freeze({
  daily: {
    id: 'daily',
    label: '日常',
...
export function getComposerMode(modeId = 'daily'): ComposerMode {
  return COMPOSER_MODES[modeId] || COMPOSER_MODES.daily;
}

export function buildComposerModeEntries(
  settings: Record<string, unknown> = {}
): Array<ComposerMode & { available: boolean; state: string; activeSkill: string }> {
...
export function getComposerModeOverrides(
  modeId = 'daily',
  settings: Record<string, unknown> = {}
): { activeSkill: string; enhance: boolean } {
  const mode = getComposerMode(modeId);
  return {
...
export function applyComposerModeToPrompt(content = '', modeId = 'daily'): string {
