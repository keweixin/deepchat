...
const SettingsPatchSchema = z.object({
  ...
  activeSkill: z.enum(['agent_auto', 'none', 'web_search', 'file_reader', 'code_runner', 'mcp_tool', 'multi_tool']).optional(),
...
const ComposerOverridesSchema = z.object({
  thinkingBudget: z.number().int().min(0).max(65536).optional(),
  activeSkill: z.enum(['agent_auto', 'none', 'web_search', 'file_reader', 'code_runner', 'mcp_tool', 'multi_tool']).optional(),
  enhance: z.boolean().optional(),
  agentMaxRounds: z.number().int().min(1).max(10).optional(),
...
const RunManualToolSchema = z.object({
